...
jubeon::LayerBase::~LayerBase()
{
}

jubeon::LayerBase::LayerBase(const LayerType layer_type, const sf::Vector2u layer_size)
	: sf::RenderTexture(), layer_size_(layer_size), layer_type_(layer_type_), is_create_buffer_(false), exit_code_(0)
{
}

bool jubeon::LayerBase::createLayerBuffer(){
	if (!this->is_create_buffer_) {
		if (!this->create(this->layer_size_.x, this->layer_size_.y)) {
			//Failed to create layer
			jubeon::Logger("LayerBase").error("Failed to create a layer.");
			return false;
		}
